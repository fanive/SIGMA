...
class FmpService {
  ...
  Future<List<dynamic>> getIncomeStatement(String ticker,
    ...
    final q = (data['quarterly']?['income_statement'] as List?) ?? [];
    final a = (data['annual']?['income_statement'] as List?) ?? [];
  ...
  Future<List<dynamic>> getBalanceSheet(String ticker,
      {int limit = 5}) async {
    final data = await SigmaApiService.getFinancials(ticker);
    final q = (data['quarterly']?['balance_sheet'] as List?) ?? [];
    final a = (data['annual']?['balance_sheet'] as List?) ?? [];
    return [...q, ...a].take(limit).toList();
  }
  ...
  Future<List<dynamic>> getCashFlowStatement(String ticker,
      {int limit = 5}) async {
    final data = await SigmaApiService.getFinancials(ticker);
    final q = (data['quarterly']?['cash_flow'] as List?) ?? [];
    final a = (data['annual']?['cash_flow'] as List?) ?? [];
    return [...q, ...a].take(limit).toList();
  }
  ...
  Future<List<dynamic>> getInstitutionalHolders(String ticker) async {
    ...
    return (data['institutional'] as List?) ?? [];
  }
  ...
  Future<String> getFmpContext(String ticker) async {
    try {
      ...
      final instHolders =
          (ownership['institutional'] as List?)?.take(5).toList() ?? [];
      if (instHolders.isNotEmpty) {
        buffer.writeln('\n=== INSTITUTIONAL_HOLDERS ===');
        for (var h in instHolders) {
          buffer.writeln(
              'HOLDER: ${h['holder'] ?? h['name']}, '
              'SHARES: ${h['shares']}, '
